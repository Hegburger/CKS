        ...
        if (mouse_press(265+65, 128, 265+65+280, 128+height) == 1) {
            ...
            if ((fr = fopen("list.dat", "rb")) == NULL) {
                ...
                while (fread(&x, sizeof(user), 1, fr)) {
                    if ((strcmp(x.phone, input_name) == 0 || strcmp(x.id, input_name) == 0) &&
                ...
                if (login_success) {
                    puthz(310, 128+height*2+gap*2+5, "登录成功", 16, 16, GREEN);
                    delay(1000);
                    return 2;  // 登录成功，跳转到主界面
                } else {
                    puthz(310, 128+height*2+gap*2+5, "手机号用户名或密码错误", 16, 16, RED);
                }
            }
...
void login_screen() {
    ...
    puthz(290, 128+height*2+gap*2, "确定", 32, 32, LIGHTRED);
    puthz(290+70+90, 128+height*2+gap*2, "注册", 32, 32, LIGHTRED);
}
